...
import { S as Subscribable, s as shallowEqualObjects, t as hashKey, v as getDefaultState, w as notifyManager, x as useQueryClient, r as reactExports, y as noop, z as shouldThrowError } from "./index-cB7zv8lW.js";
...
function usePlaceOrder() {
  ...
  return useMutation({
    mutationFn: async (request) => {
      ...
      const progressOrder = (id, delay, nextStatus) => {
        setTimeout(() => {
      ...
      progressOrder(order.id, 15e3, "preparing");
      progressOrder(order.id, 45e3, "onTheWay");
      progressOrder(order.id, 9e4, "delivered");
      return order;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["orders"] });
...
function useAdvanceOrderStatus() {
  const queryClient = useQueryClient();
  return useMutation({
